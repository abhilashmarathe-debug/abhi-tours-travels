...
import { Compass, Headphones, FileText, Bus, User, X, ArrowRight } from 'lucide-react';
...
export default function Navbar({ activePage, setActivePage }) {
  ...
  const navItems = [
    { id: 'home', label: 'Explore Holiday Portfolios', icon: Compass },
    { id: 'bus', label: 'Abhi Bus', icon: Bus },
    { id: 'track', label: 'Booking Desk & PNR Terminal', icon: FileText },
    { id: 'grievance', label: 'Guest Support & CRM', icon: Headphones },
  ];
    ...
    <>
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-xs transition-all duration-200 font-sans">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 h-16 flex items-center justify-between gap-4">
          ...
          >
            <div className="relative w-11 h-11 rounded-xl bg-gradient-to-br from-[#0B2545] via-[#133A6B] to-[#1D4ED8] p-0.5 shadow-md group-hover:shadow-lg group-hover:shadow-blue-950/20 transition-all duration-300">
              <div className="w-full h-full rounded-[10px] bg-gradient-to-br from-[#0B2545] to-[#07192F] flex items-center justify-center relative overflow-hidden border border-white/15">
                ...
                {/* Precision Aviation Crest SVG */}
                <svg
                  viewBox="0 0 48 48"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                  className="w-7 h-7 transform group-hover:scale-110 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 transition-transform duration-300"
                >
                  <defs>
                  ...
                  />
                  <path
                    d="M37.5 10.5L19.5 28.5L16.2 27.5L34.7 12L37.5 10.5Z"
                    fill="#94A3B8"
                    opacity="0.6"
                  />
                </svg>
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-1">
                <span className="text-xl font-black tracking-tight text-[#0B2545] leading-none font-mono">
                  ABHI
                </span>
                <span className="text-xl font-black tracking-tight text-[#FF9900] leading-none font-mono">
                  WORLD
                </span>
              </div>
              <span className="text-[8.5px] uppercase font-extrabold text-slate-400 tracking-[0.22em] mt-0.5">
                Escorted Luxury Tours
              </span>
            </div>
          </div>

          {/* Central Segmented Pill Controller */}
          ...
          {/* Right Action: Login / Sign Up CTA */}
          <div className="flex items-center gap-3 shrink-0">
            {isLoggedIn ? (
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-slate-800">
                  ...
                  <span className="truncate max-w-[110px]">{authForm.name || 'Guest User'}</span>
                </div>
                <button
                  onClick={handleLogout}
                  className="text-[11px] font-bold text-slate-500 hover:text-rose-600 px-2 py-1 transition cursor-pointer"
                >
                  Logout
                </button>
              </div>
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#0B2545] hover:bg-[#07192F] text-white text-xs font-black uppercase tracking-wider transition shadow-sm hover:shadow cursor-pointer active:scale-95"
              >
        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                ...
                <span className="text-[11px] text-slate-400">
                  {authMode === 'login' 
                    ? 'Access your PNR bookings & itineraries' 
            ...
            <div className="text-[10px] text-center text-slate-400">
              Demo access • Any phone number & password will authenticate
            </div>
          </div>
        </div>
      )}
